...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
Mobile API for Mind Whispers. Send `Accept: application/json` on every request.

## Authentication

1. Call **Register** or **Login** with a stable `device_name` (for example the phone model or a generated UUID for that install).
2. Persist `token` and send `Authorization: Bearer {token}` on later requests (including **Me**).
...
- **Guest** — `GET /posts`, `GET /posts/{id}` (published only), `GET /categories`, `GET /tags`. Premium `content` is `null`.
...
Same pattern as categories. Website `/tags` screens for write.
...
MD,
    ...
    'ui' => [
        'title' => env('APP_NAME', 'API').' Mobile API',
    ],
